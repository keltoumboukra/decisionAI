...
async function fetchPurchaseData(options: string): Promise<string> {
  ...
  if (isCarDecision) {
    ...
    for (const make of topMakes) {
      ...
      if (data?.results) {
        const total = data.results.reduce((sum: number, r: any) => sum + (r.numberOfComplaints ?? 0), 0);
...
async function fetchTechData(options: string): Promise<string> {
...
async function fetchTravelData(options: string): Promise<string> {
  const terms = options.split(/[,/]/).map(o => o.trim()).filter(Boolean).slice(0, 3);
  const results: string[] = [];
  for (const term of terms) {
    ...
    if (Array.isArray(data) && data[0]) {
      const c = data[0];
      results.push(`${c.name.common}: capital ${c.capital?.[0] ?? "N/A"}, region ${c.region}, population ${c.population?.toLocaleString()}`);
    }
  }
  return results.length ? results.join("\n") : "No external data available";
}

async function fetchCareerData(options: string): Promise<string> {
  ...
  const jobs = data.jobs.map((j: any) => `${j.title} at ${j.company_name} (${j.job_type})`).join("; ");
  return `Current remote job market for "${term}": ${jobs}`;
}

async function fetchFoodData(options: string): Promise<string> {
...
export async function fetchExternalData(decisionType: string, options: string): Promise<string> {
  try {
