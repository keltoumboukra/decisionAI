...
Notion Custom Agent  ← AI reasoning here (Notion credits consumed)
  ...
  returns: title, options, criteria, decisionType, urgency,
           profile, externalData (personal data sources + Notion page snippets)
        ↓
...
`externalData` is a combined string built from two parallel fetches:
1. **Personal data sources** — a `personalDataSources` array in `src/index.ts`, each entry a `() => Promise<string>` function. Currently two sources are wired up:
   - `fetchGitHubSummary()` — top 8 public repos for `keltoumboukra` fetched live from GitHub API (name, language, stars, description)
   - `fetchAppleHealthSummary()` — **mocked** Apple Health stats (steps, calories, workouts, sleep, heart rate). Replace with a real fetch from Health Auto Export or a Notion page once the pipeline is set up. To add more sources, append a new function to the `personalDataSources` array.
2. **Notion workspace pages** — `context.notion.search()` finds up to 4 pages matching the decision title + options + type; `pages.retrieveMarkdown()` fetches their content (500 char snippets each)

Both are joined and returned as `externalData`. The profile page text is returned separately as `profile`.
...
## Personal data sources (personalDataSources array in src/index.ts)
| Source | Status | Notes |
|--------|--------|-------|
| `fetchGitHubSummary()` | Live | GitHub public repos API, no auth |
| `fetchAppleHealthSummary()` | Mocked | Replace with Health Auto Export fetch or Notion page read |

To add a new source: write a `() => Promise<string>` function and append it to the `personalDataSources` array.
...
- Personal data sources (`personalDataSources` array): GitHub live + Apple Health mocked ✅
